...
namespace culebrita
{
    class pcc
    {
        ...
        /// //////////////////////////////////////
        /// </summary>
        /// <param name="culebra"></param>
        /// <param name="posiciónObjetivo"></param>
        /// <param name="longitudCulebra"></param>
        /// <param name="screenSize"></param>
        /// <returns></returns>

        private static bool MoverLaCulebrita(cc culebra, Point posiciónObjetivo, //aqui
            int longitudCulebra, Size screenSize)
        {
            var lastPoint = (Point)culebra.finalCola(); //aqui

            if (lastPoint.Equals(posiciónObjetivo)) return true;


            if (culebra.ToString().Any(x => x.Equals(posiciónObjetivo))) return false; //aqui
            ...
            // Quitar cola
            if (culebra.numeleme() > longitudCulebra) //aqui
            {
                var removePoint = (Point)culebra.quitar(); //aqui
                Console.BackgroundColor = ConsoleColor.Black;
        ...
        private static Point MostrarComida(Size screenSize, cc culebra)
        {
            var lugarComida = Point.Empty;
            var cabezaCulebra = (Point)culebra.finalCola(); //aqui
            var rnd = new Random();
            var centinelaX = cabezaCulebra.X; //aqui
            var centinelaY = cabezaCulebra.Y; //aqui
            do
            {
                var x = rnd.Next(0, screenSize.Width - 1);
                var y = rnd.Next(0, screenSize.Height - 1);
                if (culebra.ToString().All(p => centinelaX != x || centinelaY != y) //aqui
        ...
        public void Main()
        {
            ...
            var punteo = 0;
            var velocidad = 80; //modificar estos valores y ver qué pasa
            var posiciónComida = Point.Empty;
            var tamañoPantalla = new Size(60, 20);
            var culebrita = new cc();
